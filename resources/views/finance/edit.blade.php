...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-text-main dark:text-text-darkMain">Edit Transaksi</h1>
</div>

<div class="max-w-xl">
    <div class="pcard animate-fade-in-up">
        <form method="POST" action="{{ route('finance.update', $finance) }}" id="edit-finance-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex-1 cursor-pointer group">
                        <input type="radio" name="type" value="income" class="hidden peer type-radio" {{ old('type', $finance->type) === 'income' ? 'checked' : '' }}>
                    ...
                    <label class="flex-1 cursor-pointer group">
                        <input type="radio" name="type" value="expense" class="hidden peer type-radio" {{ old('type', $finance->type) === 'expense' ? 'checked' : '' }}>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" id="amount" name="amount" class="form-control-p pl-9 font-semibold" value="{{ old('amount', $finance->amount) }}" placeholder="0" min="0.01" step="0.01" required>
                </div>
            ...
            <div>
                ...
                <input type="text" id="description" name="description" class="form-control-p" value="{{ old('description', $finance->description) }}" placeholder="Contoh: Gaji, Makan siang…">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label for="date" class="form-label-p">Tanggal <span class="text-red-500">*</span></label>
                    <input type="date" id="date" name="date" class="form-control-p" value="{{ old('date', $finance->date) }}" required>
                </div>
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id" class="form-control-p form-select-p">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ (string) old('category_id', $finance->category_id) === (string) $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea id="notes" name="notes" class="form-control-p" rows="2" placeholder="Catatan tambahan (opsional)">{{ old('notes', $finance->notes) }}</textarea>
            </div>
